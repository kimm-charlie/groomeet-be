...
### 3. pgvector 의미 유사도 검색 PoC

키워드 검색이 못 잡는 의미 유사 포트폴리오를 임베딩(OpenAI `text-embedding-3-small` 1536차원) + pgvector HNSW cosine으로 검색. 운영 MySQL을 건드리지 않도록 PostgreSQL 벡터 저장소를 분리한 멀티 데이터소스 구성.

> 별도 브랜치 [`poc/pgvector-semantic-search`](../../tree/poc/pgvector-semantic-search) · [main과 비교](../../compare/main...poc/pgvector-semantic-search)
> 핵심 코드: PoC 브랜치의 `module/member/portfolio_embedding/`, `common/config/PostgresDataSourceConfig.java`
