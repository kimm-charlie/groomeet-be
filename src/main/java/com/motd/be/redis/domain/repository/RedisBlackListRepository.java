...
public class RedisBlackListRepository {
	...
	/**
	 * 로그아웃된 토큰 저장
	 * key: auth:access-token:blacklist:sign_out:{token-hash}
	 * value: "blacklisted"
	 */
	public void setBlackListForSignOut(String accessToken) {
		String tokenHash = DigestUtils.md5DigestAsHex(accessToken.getBytes());
		String key = BLACKLIST_FOR_SIGN_OUT_PREFIX + tokenHash;
	 ...
	 * 블랙리스트 포함 여부 확인
	 */
	public boolean isBlackListTokenForSignOut(String accessToken) {
		String tokenHash = DigestUtils.md5DigestAsHex(accessToken.getBytes());
		String key = BLACKLIST_FOR_SIGN_OUT_PREFIX + tokenHash;
